...
class KDTree
{
  ...
  private:
    ...
    KDTreeNode* generateTree(int left, int right, int dim);

    void swap(int idx1, int idx2);

    int partition(int left, int right, int pivot_idx, int dim);

    void select(int left, int right, int k, int dim);

    void clear(KDTreeNode* subroot);

    double dist(const Point<Dim>& first, const Point<Dim>& second) const;

    Point<Dim> findNearestNeighbor_helper(KDTreeNode* subroot, const Point<Dim> target, Point<Dim> currBest, int dim) const;
};
