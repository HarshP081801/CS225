...
size_t insertion_idx(const std::vector<T>& elements, const C& val, int begin, int end) {

    // if the begin and end are the only two elements in the element, return end;
    if (begin == end - 1 && val > elements[begin] && val < elements[end]) return end;

    // the middle value of the sorted vector
    T midpoint = elements[(begin + end) / 2];

    // if  the value is the same as the middle, return the index of the middle
    if (val == midpoint) return (begin + end) / 2;

    // if the value is larger than the middle, recurse through the second half of the vector 
    if (val > midpoint) return insertion_idx(elements, val, (begin + end) / 2, end);

    // if the value is smaller than the middle, recruse through the first half of the vector
    if (val < midpoint) return insertion_idx(elements, val, begin, (begin + end) / 2);
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    //if val is smaller than the smallest value in the vector
    if (elements[0] > val) return 0;

    // if val is larger than the biggest value in the vector
    if (elements[elements.size() - 1] < val) return elements.size();

    // if val is equal to the largest value in the vector
    if (elements[elements.size() - 1] == val) return elements.size() - 1;

    // if none of these, recurse through the helper function and find the index
